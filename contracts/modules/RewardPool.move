module RewardPool {
    ...
    fun _earned<StakeToken, RewardToken>(
        ...
        user_stake.rewards +
            Math::mul_div(
                user_stake.stake,
                reward_per_token - user_stake.reward_per_token_paid,
                Math::pow(2, MULTIPLER),
            )
    }
    ...
    fun _reward_per_token<StakeToken, RewardToken>(pool: &T<StakeToken, RewardToken>): u128 {
        ...
        if (total_staked == 0) {
            ...
            let reword_per_token_from_last = Math::mul_div(Math::pow(2, MULTIPLER), (duration_from_last_time as u128) * pool.reward_rate, total_staked);
